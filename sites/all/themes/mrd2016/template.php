<?php
function mrd2016_preprocess_page(&$vars, $hook) {
 ...
 */
function mrd2016_preprocess_node(&$vars) {
  if($vars['view_mode'] == 'carousel_item') {
    $vars['theme_hook_suggestions'][] = 'node__' . $vars['type'] . '__carousel_item';
  }
  if($vars['view_mode'] == 'grid_item') {
    $vars['theme_hook_suggestions'][] = 'node__' . $vars['type'] . '__grid_item';
  }
...
function mrd2016_preprocess_image(&$vars) {
